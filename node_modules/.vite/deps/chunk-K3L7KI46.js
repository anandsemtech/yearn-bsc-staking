...
function ecdsa(Point2, hash9, ecdsaOpts = {}) {
  ...
  const hmac2 = ecdsaOpts.hmac || ((key, ...msgs) => hmac(hash9, key, concatBytes(...msgs)));
  ...
  function prepSig(message, privateKey, opts) {
    ...
    if (extraEntropy3 != null && extraEntropy3 !== false) {
      const e = extraEntropy3 === true ? randomBytes2(lengths.secretKey) : extraEntropy3;
  ...
  function sign7(message, secretKey, opts = {}) {
    ...
    const drbg = createHmacDrbg(hash9.outputLen, Fn.BYTES, hmac2);
...
function createSWU(Point2, opts) {
  const map = mapToCurveSimpleSWU2(Point2.Fp, opts);
...
(function(encodePacked3) {
  function encode11(type6, value, isArray = false) {
    ...
    if (arrayMatch && Array.isArray(value)) {
      ...
      const data = [];
      for (let i = 0; i < value.length; i++) {
        data.push(encode11(childType, value[i], true));
      }
      if (data.length === 0)
        return "0x";
      return concat4(...data);
    ...
    if (args_.length > 0) {
      ...
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i) => {
        if (Array.isArray(args_[i]))
          return args_[i].map((_, j) => encode11(param, args_[i][j]));
        return typeof args_[i] !== "undefined" && args_[i] !== null ? encode11(param, args_[i]) : null;
      })) ?? [];
    }
  }
  const selector = (() => {
...
function fromString6(value) {
  return from15(fromString3(value));
}
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b, from32, to3) => bytesToNumberBE(b.slice(from32, to3));
...
function sign4(options) {
  ...
  const payloadPoint = payloadGroup.hashToCurve(from(payload), suite ? { DST: fromString3(suite) } : void 0);
...
function verify4(options) {
  ...
  const payloadPoint = group.hashToCurve(from(payload), suite ? { DST: fromString3(suite) } : void 0);
...
function fromCreate(options) {
  const from32 = fromHex(from6(options.from));
  let nonce = fromNumber3(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from6(`0x${keccak2563(fromBytes5([from32, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  ...
  return from6(slice3(keccak2563(concat3(fromHex("0xff"), from32, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function namehash(name) {
  ...
  for (let i = labels.length - 1; i >= 0; i -= 1) {
    ...
    const hashed = hashFromEncodedLabel ? fromHex(hashFromEncodedLabel) : keccak2563(fromString3(labels[i]), { as: "Bytes" });
...
function serialize6(value) {
  ...
  return stringify3({ domain, message, primaryType, types: types2 }, (_, value2) => {
    if (typeof value2 === "bigint")
...
// node_modules/viem/node_modules/ox/_esm/erc4337/UserOperation.js
function from31(userOperation, options = {}) {
  const signature = (() => {
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-K3L7KI46.js.map
