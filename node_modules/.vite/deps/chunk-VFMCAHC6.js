...
var useAffectedDebugValue = (state18, affected) => {
  ...
  (0, import_react.useEffect)(() => {
    pathList.current = affectedToPathList(state18, affected, true);
  });
...
var EventsController = {
  ...
  _setPendingEvent(payload) {
    ...
    try {
      ...
      if (JSON.stringify(state6.pendingEvents).length / 1024 > MAX_PENDING_EVENTS_KB) {
  ...
  _submitPendingEvents() {
    if (state6.pendingEvents.length === 0) {
      return;
    }
    try {
      api2.sendBeacon({
    ...
    if (ChainController.state.activeChain === ConstantsUtil.CHAIN.EVM) {
      const rdnsCandidates = [
        ...connectors.flatMap((c) => {
          var _a;
          return ((_a = c.connectors) == null ? void 0 : _a.map((sc) => {
            var _a2;
...
var controller4 = {
  ...
  goBack() {
    ...
    if ((_b = state10.data) == null ? void 0 : _b.redirectView) {
...
var controller8 = {
  ...
  async handleAuthAccountSwitch({ address, namespace }) {
    var _a, _b;
    const accountData = ChainController.getAccountData(namespace);
    const smartAccount = (_b = (_a = accountData == null ? void 0 : accountData.user) == null ? void 0 : _a.accounts) == null ? void 0 : _b.find((c) => c.type === "smartAccount");
...
async function loadViemUtils() {
  if (!cachedViemUtils) {
    const { createPublicClient, http, defineChain } = await import("./_esm-3Y6HXJST.js");
    cachedViemUtils = {
      createPublicClient,
      http,
      defineChain
...
var controller9 = {
  ...
  getSdkEventProperties(error) {
    ...
    return {
      ...
      token: ((_a = state15.token) == null ? void 0 : _a.symbol) || "",
      amount: state15.sendTokenAmount ?? 0,
...
var controller10 = {
  ...
  subscribeAccountStateProp(property, callback, chain) {
    ...
    return subscribeKey(((_a = state16.chains.get(activeChain)) == null ? void 0 : _a.accountState) || {}, property, callback);
  },
  ...
  setActiveCaipNetwork(caipNetwork) {
    ...
    const isSameNamespace = state16.activeChain === caipNetwork.chainNamespace;
    if (!isSameNamespace) {
    ...
    if (address) {
      state16.activeCaipAddress = `${caipNetwork.chainNamespace}:${caipNetwork.id}:${address}`;
    } else if (isSameNamespace && state16.activeCaipAddress) {
      const { address: parsedAddress } = ParseUtil.parseCaipAddress(state16.activeCaipAddress);
      address = parsedAddress;
      state16.activeCaipAddress = `${caipNetwork.caipNetworkId}:${address}`;
    } else {
      state16.activeCaipAddress = void 0;
    }
    ChainController.setChainAccountData(caipNetwork.chainNamespace, {
      address,
      caipAddress: state16.activeCaipAddress
    });
    SendController.resetSend();
    PublicStateController.set({
      activeChain: state16.activeChain,
      selectedNetworkId: (_b = state16.activeCaipNetwork) == null ? void 0 : _b.caipNetworkId
    });
...
//# sourceMappingURL=chunk-VFMCAHC6.js.map
