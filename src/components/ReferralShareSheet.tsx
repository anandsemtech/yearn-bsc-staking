...
// 🔹 SAME env var used by StakingModal.tsx
...
const ReferralSection: React.FC<Props> = ({
  ...
  // public client (wagmi -> fallback), same idea as in StakingModal
  ...
  // 🔹 eligibility (whitelisted or staked > 0) — FAIL-CLOSED on any error/invalid env
  ...
  useEffect(() => {
    ...
    // Debug: boot info
    console.debug(LOG, "boot", {
      STAKING_CONTRACT,
      stakingContractValid: stakingOk,
      connectedAddress: address ?? null,
    });

    (async () => {
      if (!addr || !stakingOk) {
        console.debug(LOG, "eligibility: early-exit (no addr or invalid env)", { addr, stakingOk });
        if (!stop) setEligible(false);
        return;
      }

      try {
        console.debug(LOG, "eligibility: reading on-chain", { addr, STAKING_CONTRACT });
        ...
        const isEligible = Boolean(wl) || (staked ?? 0n) > 0n;

        console.debug(LOG, "eligibility: results", {
          connectedAddress: addr,
          isWhitelisted: wl,
          totalStaked: staked?.toString?.() ?? String(staked),
          eligible: isEligible,
        });

        if (!stop) setEligible(isEligible);
      } catch (e) {
        console.debug(LOG, "eligibility: read failed, fail-closed", {
          connectedAddress: addr,
  ...
  const link = useMemo(() => {
    const v = (origin && address && eligible) ? `${origin}?ref=${address}` : "";
    console.debug(LOG, "share-link", {
      origin,
      connectedAddress: address ?? null,
      eligible,
      link: v || "(empty)",
            ...
            <div className="text-xs font-semibold text-white mb-1 flex items-center gap-2">
              ...
              {eligible === false && (
                <span className="ml-2 text-[11px] text-amber-300/90">
                  (Stake or get whitelisted to unlock)
                </span>
              )}
            </div>
